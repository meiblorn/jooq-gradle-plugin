...
class GenericDatabaseContainerTest {
    ...
    @MockK private lateinit var jdbcAwareClassLoader: ClassLoader
    ...
    fun `should only load driver class once`() {
        ...
        // then
        verify(exactly = 1) { jdbcAwareClassLoader.loadClass(database.jdbc.driverClassName) }
    }
    ...
    fun `should rethrow expected exceptions as no driver found exception`() =
        sequenceOf(
            ...
            .map { exception ->
                dynamicTest(
                    "should rethrow ${exception::class.simpleName} as no driver found exception") {
                        // given
                        every { jdbcAwareClassLoader.loadClass(any()) } throws exception

                        // when && then
                        expectThrows<NoDriverFoundException> { container.jdbcDriverInstance }
    ...
    fun `should rethrow unexpected exceptions as is`() {
        ...
        // when && then
        expectThrows<RuntimeException> { container.jdbcDriverInstance } isEqualTo
            unexpectedException
    }
