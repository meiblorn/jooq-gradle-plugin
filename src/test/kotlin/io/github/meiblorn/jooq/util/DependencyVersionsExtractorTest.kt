...
class DependencyVersionsExtractorTest {
    ...
    fun `should throw exception if the specified resource does not exist`() {
        // when && then
        expectThrows<IllegalArgumentException> { DependencyVersionsExtractor("/does/not/exist") }
    ...
    fun `should throw exception if the requested version does not exist`() {
        // given
        val extractor =
            DependencyVersionsExtractor("/DependencyVersionsExtractorTest/test.versions")

        // when
        val nonExistentVersion by extractor

        // then
        expectThrows<IllegalArgumentException> { nonExistentVersion }.message isEqualTo
            "Dependency version for nonExistentVersion not found"
    }
