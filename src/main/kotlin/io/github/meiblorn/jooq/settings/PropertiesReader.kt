...
internal object PropertiesReader {
    const val PREFIX = "io.github.meiblorn.jooq."
    ...
    fun WithoutContainer.applyPropertiesFrom(
        pluginProperties: Map<String, String>
    ): JooqDockerPluginSettings = apply { database.applyPropertiesFrom(pluginProperties) }
    ...
    private fun Image.applyPropertiesFrom(pluginProperties: Map<String, String>) {
        ...
        pluginProperties
            ...
            .map { (key, value) -> key.removePrefix(envVarsPrefix) to value.toString() }
            .takeIf { it.isNotEmpty() }
            ?.also { envVars = it.toMap() }
    }
    ...
    private fun <T> Map<String, String>.findAndSetProperty(
        ...
        get("$prefix${property.name}")?.also { property.set(mapper(it)) }
    }
