...
internal class ReflectiveJooqCodegenRunner(codegenAwareClassLoader: ClassLoader) :
    JooqCodegenRunner {
    ...
    private class ReflectiveGenerationTool(codegenAwareClassLoader: ClassLoader) {
        ...
        private val configurationClass =
            codegenAwareClassLoader.loadClass(Configuration::class.jvmName)
        ...
        fun setClassLoader(classLoader: ClassLoader) {
            val setClassLoaderMethod =
                toolClass.getDeclaredMethod(
                    GenerationTool::setClassLoader.name, ClassLoader::class.java)
            ...
            ByteArrayOutputStream()
                .also { stream ->
                    stream.writer().use { writer ->
                        @Suppress("UnstableApiUsage") MiniJAXB.marshal(this, writer)
                        writer.flush()
                    }
                }
                .toByteArray()
        ...
        private fun load(xmlByteArray: ByteArray): Any {
            ...
            return ByteArrayInputStream(xmlByteArray).use { loadMethod.invoke(null, it) }
        }
    }
}
