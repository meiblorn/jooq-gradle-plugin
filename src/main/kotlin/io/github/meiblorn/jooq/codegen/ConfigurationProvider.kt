...
internal class ConfigurationProvider(
    ...
    fun defaultConfiguration() =
        Generator()
            ...
            .let { Configuration().withGenerator(it) }
    ...
    private fun Configuration.applyCommonConfiguration() = also { config ->
        config.generator.apply {
            ...
            schemaToPackageMapping.get().toMatchersStrategy()?.also { matchers ->
                withStrategy(Strategy().withMatchers(matchers))
            }
        }
    }
    ...
    private fun Map<String, String>.toMatchersStrategy() =
        takeIf { it.isNotEmpty() }
            ?.let {
                Matchers()
                    .withSchemas(*it.toSchemasMatchers())
                    .withCatalogs(*it.toCatalogMatchers())
            }
    ...
    private fun Map<String, String>.toSchemasMatchers() =
        map { (schema, pkg) ->
                MatchersSchemaType()
                    ...
                    .withSchemaIdentifier(MatcherRule().withTransform(AS_IS).withExpression(pkg))
            }
            .toTypedArray()
    ...
    private fun Map<String, String>.toCatalogMatchers() =
        map { (schema, pkg) ->
                MatchersCatalogType()
                    .withExpression(schema)
                    .withCatalogIdentifier(MatcherRule().withTransform(AS_IS).withExpression(pkg))
            }
            .toTypedArray()
