...
abstract class FunctionalTestBase {
    ...
    protected fun writeProjectFile(fileName: String, bodySupplier: () -> String) =
        projectFile(fileName).writeText(bodySupplier())

    protected fun projectFile(fileName: String) =
        File(projectDir, fileName).also { it.parentFile.mkdirs() }

    protected fun Assertion.Builder<BuildResult>.getTask(taskName: String) =
        get { task(":$taskName") }.describedAs("Task $taskName")

    protected val Assertion.Builder<BuildTask?>.outcome
        get() = isNotNull().get { outcome }

    protected fun Assertion.Builder<BuildResult>.getTaskOutcome(taskName: String) =
        getTask(taskName).outcome

    protected val Assertion.Builder<BuildResult>.generateJooqClassesTask
        get() = getTask("generateJooqClasses")
}
